...
function validateComponent() {
  ...
  then
    if grep -q '<name>ApexClass</name>' $PACKAGE_XML; then
          ...
          sf project deploy start -o $TARGET_ORG_ALIAS -x $PACKAGE_XML -l RunSpecifiedTests --tests HelloWorldTest --verbose --dry-run --async --ignore-conflicts | tee $RESULT_FILE 
      else
